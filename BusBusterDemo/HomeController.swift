...
class HomeController: UIViewController {
	...
	private let calculateRoadButton = UIButton()
    private let tableView = UITableView()
    private var searchResults = [MKPlacemark]()
    private var savedLocations = [MKPlacemark]()
    private final let locationInputViewHeight: CGFloat = 120
	...
	var groupedRoutes: [(startItem: MKMapItem, endItem: MKMapItem)] = []
...
//		groupAndRequestDirections()
		...
		myLocationButton.anchor(bottom: view.bottomAnchor, right: view.rightAnchor, paddingBottom: 50, paddingRight: 15, width: 40, height: 40)
        myLocationButton.setImage(#imageLiteral(resourceName: "nav").withRenderingMode(.alwaysOriginal), for: .normal)
        myLocationButton.addTarget(self, action: #selector(centerMapOnUserLocation), for: .touchUpInside)
    }

	func configurateMapViewForDriver() {
		view.addSubview(mapView)
				mapView.frame = view.frame
	...
	func configureRoadButton() {
		view.addSubview(calculateRoadButton)
		calculateRoadButton.setImage(#imageLiteral(resourceName: "bus").withRenderingMode(.alwaysOriginal), for: .normal)
		calculateRoadButton.anchor(bottom: view.bottomAnchor, right: view.rightAnchor, paddingBottom: 100, paddingRight: 15, width: 40, height: 40)
		calculateRoadButton.addTarget(self, action: #selector(calculateRoadForDriver), for: .touchUpInside)
    }

	@objc func calculateRoadForDriver() {
		#warning("Прорисовка маршрута")
		makeRoad()
	}
	...
	func hideTextBar() {
		locationInputView.isHidden = true
	}
...
extension HomeController: PickupControllerDelegate {
	...
	#warning("func постройки маршрута")
	func makeRoad() {

		// Чутка подвинули к первому отелю : 55,752803, 37,609480
		// Красная площадь:    55,755786 ,  37,617633
		// Координаты первого отеля: 55,751071, 37,600898



//		var pickupCoordinates: CLLocationCoordinate2D!
		var firstStopCoordinates: CLLocationCoordinate2D!
		var secondStopCoordinates: CLLocationCoordinate2D!
		var excursionCoordinates: CLLocationCoordinate2D!

		#warning("Мой локейшн")
		let myMapItem = MKMapItem.forCurrentLocation()

//		guard let lat = -26.2041028 as? CLLocationDegrees else { return }
//		guard let long =  28.0473051 as? CLLocationDegrees else { return }
		firstStopCoordinates = CLLocationCoordinate2D(latitude: 55.751071, longitude: 37.600898)
//		55.751071, 37.600898

//		guard let lat1 = -26.2041028 as? CLLocationDegrees else { return }
//		guard let long1 =  28.0473051 as? CLLocationDegrees else { return }
		secondStopCoordinates = CLLocationCoordinate2D(latitude: 55.740607, longitude: 37.617115)

//		55.740607, 37.617115

		self.mapView.addAnnotationAndSelect(forCoordinate: firstStopCoordinates)
		self.mapView.addAnnotationAndSelect(forCoordinate: secondStopCoordinates)

		#warning("Перевод 2DCoordinat в MKMapItem")
		let placemark = MKPlacemark(coordinate: firstStopCoordinates)
		let mapItemFirstStop = MKMapItem(placemark: placemark)

		let placemark1 = MKPlacemark(coordinate: secondStopCoordinates)
		let mapItemSecondStop = MKMapItem(placemark: placemark1)

		// 55.060877, 82.905338 - наша локация

//		groupedRoutes.append((mapItemFirstStop, mapItemSecondStop))
//		groupedRoutes.append((mapItemSecondStop, myMapItem))


//		var route = Route(origin: mapItemFirstStop, stops: [mapItemFirstStop, mapItemSecondStop])

		groupedRoutes.append((myMapItem, mapItemFirstStop))

		groupedRoutes.append((mapItemFirstStop, mapItemSecondStop))
		groupedRoutes.append((mapItemSecondStop, mapItemFirstStop)) //, route.origin
...
//		  self.activityIndicatorView.stopAnimating()
	...
	private func updateView(with mapRoute: MKRoute) {
		...
		print("Дистанция: \(totalDistance), \(totalTravelTime)")
